...
ADMIN_IDS = {7262941693, 8505301090}
...
# Render.com-da bu avtomatik olaraq RENDER_EXTERNAL_URL env dəyişənində gəlir —
# başqa yerdə (məs. PythonAnywhere) işlədirsənsə, aşağıdakı defolt dəyəri özün dəyiş.
WEBHOOK_PUBLIC_URL = os.environ.get("RENDER_EXTERNAL_URL", "https://sizin-domain.onrender.com")
...
def admin_id_dir(user_id):
    return user_id in ADMIN_IDS


def adminlere_mesaj(text):
    for aid in ADMIN_IDS:
        try:
            bot.send_message(aid, text)
        except Exception as e:
            logger.error(f"Admin ({aid}) mesajı göndərilə bilmədi: {e}")


def adminlere_foto(file_id, caption, reply_markup=None):
    for aid in ADMIN_IDS:
        try:
            bot.send_photo(aid, file_id, caption=caption, reply_markup=reply_markup)
        except Exception as e:
            logger.error(f"Admin ({aid}) foto göndərilə bilmədi: {e}")
...
def odenis_ugurlu_emal_et(order_ref):
    ...
    if not netice.get("success"):
        ...
        adminlere_mesaj(f"‼️ Sifariş #{row['id']} EpinBulk xətası: {xeta}\nÖdəniş geri qaytarılmalıdır.")
        return
...
def gozleyen_sifarisi_yoxla(row):
    ...
    if status == "COMPLETED":
        ...
        adminlere_mesaj(f"‼️ Sifariş #{row['id']} {status} oldu — geri ödəmə lazımdır.")
...
def admin_emri(message):
    if not admin_id_dir(message.from_user.id):
...
def admin_paneli_goster(chat_id):
    ...
    if balans is not None and balans < MIN_BALANCE_ALERT:
        adminlere_mesaj(f"⚠️ Balans azdır! Cari balans: {balans:.2f} USD")
...
def odenis_novu_enot_basla(call, order_ref):
    ...
    if not odeniş_url:
        ...
        adminlere_mesaj(f"‼️ Enot.io invoice yaradıla bilmədi — sifariş {order_ref}")
        return
...
def manual_odenisi_tesdiqle(call, order_ref):
    ...
    if not admin_id_dir(call.from_user.id):
