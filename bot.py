...
#   - Yığım.az ilə ödəniş linki yaratma və webhook qəbulu
...
YIGIM_API_KEY = "YIGIM_KEY"
ADMIN_ID = 7262941693

# /admin, /kart_bax, /kart_deyis əmrlərindən əvvəl bu istifadəçi adı/parol tələb olunur.
ADMIN_USERNAME = "bossepinadmini"
ADMIN_PASSWORD = "senedexliyoxdu"

EPINBULK_BASE_URL = "https://epinbulk.shop/api/v1"

# Yığım.az-ın açıq (public) API sənədi yoxdur — bu dəyərləri öz
# Yığım mərçant panelinizdən / müqavilə sənədinizdən əldə edin.
YIGIM_BASE_URL = "https://api.yigim.az"            # DƏQİQLƏŞDİRİN
YIGIM_MERCHANT_ID = "YIGIM_MERCHANT_ID"            # DƏQİQLƏŞDİRİN
...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS sifarisler (
            ...
            player_id TEXT,
            status TEXT,
            odenis_novu TEXT DEFAULT 'yigim',
    ...
    for sutun, tip in (("odenis_novu", "TEXT DEFAULT 'yigim'"), ("manual_qebz_file_id", "TEXT")):
        try:
...
# ----------------------------------------------------------------
# YIĞIM.AZ ÖDƏNİŞ FUNKSİYALARI
# QEYD: Yığım-ın açıq API sənədi yoxdur (yalnız müqavilə əsasında verilir).
# Aşağıdakı sahə adları ÜMUMİ formatdadır — real inteqrasiyadan əvvəl
# öz Yığım sənədinizlə YOXLAYIN və lazım gələrsə DƏYİŞİN. Webhook-un xam
# gövdəsi hər zaman log.txt-ə yazılır ki, real formatı görüb uyğunlaşdıra biləsiniz.
# ----------------------------------------------------------------
def yigim_odenis_linki_yarat(order_ref, mebleg, tesvir):
    payload = {
        "merchant_id": YIGIM_MERCHANT_ID,
        "order_id": order_ref,
        "amount": round(float(mebleg), 2),
        "currency": PUL_VAHIDI,
        "description": tesvir,
        "callback_url": f"{WEBHOOK_PUBLIC_URL}/yigim-webhook",
    }
    headers = {
        "Authorization": f"Bearer {YIGIM_API_KEY}",
        "Content-Type": "application/json",
    }
    try:
        r = requests.post(f"{YIGIM_BASE_URL}/api/v1/payment-links", json=payload, headers=headers, timeout=20)
        data = r.json()
        return data.get("payment_url") or data.get("url") or data.get("link")
    except Exception as e:
        logger.error(f"Yığım ödəniş linki yaradılarkən xəta ({order_ref}): {e}")
        return None
...
# FLASK: YIĞIM WEBHOOK
# ----------------------------------------------------------------
@app.route("/yigim-webhook", methods=["POST"])
def yigim_webhook():
    xam = request.get_data(as_text=True)
    logger.info(f"Yığım webhook alındı: {xam}")
    ...
    # QEYD: sahə adlarını öz Yığım sənədinizə görə yoxlayın/dəyişin
    order_ref = data.get("order_id") or data.get("orderId") or data.get("reference")
    status = str(data.get("status", "")).lower()

    if not order_ref:
        logger.error("Webhook-da order_id/reference tapılmadı")
        return jsonify({"ok": False}), 400

    if status in ("success", "succeeded", "paid", "completed", "ok"):
        threading.Thread(target=odenis_ugurlu_emal_et, args=(order_ref,), daemon=True).start()
    else:
        logger.info(f"Ödəniş uğursuz/gözləmədə: {order_ref} -> {status}")
...
def admin_girisi_yoxla(message):
    """/kart_bax və /kart_deyis kimi digər admin əmrləri üçün: giriş edilməyibsə xəbərdar edir."""
    if message.from_user.id != ADMIN_ID:
        return False
...
def id_qebul_et(message):
    ...
    try:
        ...
        user_states[user_id] = {"stage": "manual_qebz_gozlenilir", "order_ref": order_ref}
        ...
        bot.send_message(user_id, metin)
    except Exception as e:
